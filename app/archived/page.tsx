...
import { Plus, FileText, Archive, User, Users, RotateCcw } from 'lucide-react';
...
export default function ArchivedDocumentsPage() {
  ...
  useEffect(() => {
    if (status === 'authenticated' && session?.user?.id) {
      fetch(`/api/documents/archived?userId=${session.user.id}`)
        .then(async (res) => {
  ...
  if (status === 'loading' || loading) {
    return (
      <DashboardLayout>
        <div className="text-center py-10 text-muted-foreground">Loading archived documents...</div>
      </DashboardLayout>
    );
  }
  ...
  if (status !== 'authenticated') {
    return (
      <div className="text-center py-10 text-muted-foreground">
        Please login to view archived documents.
    ...
    <DashboardLayout>
      ...
      {documents.length === 0 ? (
        <div className="text-muted-foreground text-center py-10">
          ...
          <p className="text-lg font-medium">No archived documents</p>
