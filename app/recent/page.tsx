...
import { Plus, FileText, Clock, User, Users } from 'lucide-react';
...
export default function RecentDocumentsPage() {
  ...
  useEffect(() => {
    if (status === 'authenticated' && session?.user?.id) {
      fetch(`/api/documents/recent?userId=${session.user.id}`)
        .then(async (res) => {
  ...
  if (status === 'loading' || loading) {
    return (
      <DashboardLayout>
        <div className="text-center py-10 text-muted-foreground">Loading recent documents...</div>
      </DashboardLayout>
    );
  }
  ...
  if (status !== 'authenticated') {
    return (
      <div className="text-center py-10 text-muted-foreground">
        Please login to view recent documents.
    ...
    <DashboardLayout>
      ...
      {documents.length === 0 ? (
        <div className="text-muted-foreground text-center py-10">
          ...
          <p className="text-sm">Your recently updated documents will appear here</p>
        </div>
      ) : (
