...
export default function SharedDocumentsPage() {
  ...
  useEffect(() => {
    if (status === 'authenticated' && session?.user?.id) {
      fetch(`/api/documents/shared?userId=${session.user.id}`)
        .then(async (res) => {
  ...
  if (status === 'loading' || loading) {
    return (
      <DashboardLayout>
        <div className="text-center py-10 text-muted-foreground">Loading shared documents...</div>
      </DashboardLayout>
    );
  }
  ...
  if (status !== 'authenticated') {
    return (
      <div className="text-center py-10 text-muted-foreground">
        Please login to view shared documents.
